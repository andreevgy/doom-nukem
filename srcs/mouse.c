...
/*   Updated: 2019/03/31 12:34:44 by marvin           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	mouse_movement(SDL_Event event, t_doom **doom)
{
	...
	if (event.motion.type == SDL_MOUSEMOTION)
	{
		...
		(*doom)->dir.x = (*doom)->dir.x * cos(xrel)
			- (*doom)->dir.y * sin(xrel);
		(*doom)->dir.y = old_dir_x * sin(xrel)
			+ (*doom)->dir.y * cos(xrel);
		(*doom)->plane.x = (*doom)->plane.x * cos(xrel)
			- (*doom)->plane.y * sin(xrel);
		(*doom)->plane.y = old_plane_x * sin(xrel)
			+ (*doom)->plane.y * cos(xrel);
		if (((*doom)->vertical >= -600 && yrel < 0)
		|| ((*doom)->vertical <= 600 && yrel > 0))
			(*doom)->vertical += yrel;
		return (1);
	}
	return (0);
